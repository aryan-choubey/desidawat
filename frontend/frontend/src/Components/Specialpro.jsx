...
export const Specialpro = () => {
  ...
  const unitPrice = product.price[weight]; // get actual price for selected weight
  ...
  const newItem = {
    name: product.name,
    key: product.name.toLowerCase().replace(/\s+/g, "-"), // unique key
    image: product.img,
    weight,
    quantity: qty,
    unitPrice,  
  ...
  // check if already exists (same product + weight)
  const existingIndex = cart.findIndex(
    (item) => item.key === newItem.key && item.weight === weight
  );
